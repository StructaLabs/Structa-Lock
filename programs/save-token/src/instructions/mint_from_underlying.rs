//! Instruction handler for [SVE_token::mint_from_underlying].
...
impl<'info> MintFromUnderlying<'info> {
        ...
        yi::cpi::stake(
            CpiContext::new(
                ...
                yi::cpi::accounts::Stake {
                    ...
                    destination_yi_tokens: self.common.SVE_yi_tokens.to_account_info(),
    ...
    fn stake_and_mint_to(&mut self, amount: u64) -> Result<()> {
        let prev_amount = self.common.SVE_yi_tokens.amount;
        self.stake_yi(amount)?;
        self.common.SVE_yi_tokens.reload()?;
        let next_amount = self.common.SVE_yi_tokens.amount;
        let yi_minted = unwrap_int!(next_amount.checked_sub(prev_amount));
        self.common.mint_SVE(yi_minted)?;
        Ok(())
    }
}
...
impl<'info> Validate<'info> for MintFromUnderlying<'info> {
    fn validate(&self) -> Result<()> {
        ...
        let SVE = self.common.SVE.load()?;
        let yi = self.yi.load()?;
        assert_keys_eq!(self.yi, SVE.yi);
        assert_keys_eq!(self.yi_mint, SVE.yi_mint);
        ...
        assert_keys_eq!(SVE.underlying_mint, self.source_underlying_tokens.mint);
